...
# 使用tokenizer对格式化后的问题进行编码并移动到GPU
...
model = FastLanguageModel.get_peft_model(
    ...
    use_gradient_checkpointing = "unsloth", # 使用优化技术节省现存并支持更大的批量大小
    random_state = 3407, # 设置随机种子，确保每次运行代码时模型的初始化方式相同
    use_rslora = False, # 设置是否使用Rank Stabilized LoRA技术，这里设置为False比松hi不使用
    loftq_config = None, # 设置是否使用LoftQ技术，这里设置为None表示不使用
)

from trl import SFTTrainer # 导入SFTTrainer，用于监督式微调
from transformers import TrainingArguments # 导入TrainingArguments，用于设置训练参数
from unsloth import is_bfloat16_supported # 导入函数，检查是否支持bffloat16数据格式

trainer = SFTTrainer( # 创建一个SFTTrainer实例
    ...
    args=TrainingArguments( # 定义训练参数
        ...
        fp16=not is_bfloat16_supported(), # 是否使用fp16格式加速训练
        bf16=is_bfloat16_supported(), # 是否使用bfloat16格式加速训练
...
inputs = tokenizer([promt_style.format(question, "")], return_tensors="pt").to("cuda")

# 让模型根据问题生成回答，最多生成4000个新词
outputs = models.generate(
    input_ids=inputs.input_ids, # 输入的数字序列
    attention_mask=inputs.attention_mask, # 注意力遮罩，帮助模型理解哪些部分重要
    max_new_tokens=4000, # 最多生成4000个新词
...
GUGGINGFACE_TOKEN = userdata.get('HUGGINGFACE_TOKEN')
...
# 导入Hugging Face Hub的create_repo函数，用于创建一个新的模型仓库
